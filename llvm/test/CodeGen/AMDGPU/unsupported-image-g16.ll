...
; RUN: not llc -global-isel=1 -new-reg-bank-select -mtriple=amdgcn -mcpu=fiji -filetype=null %s 2>&1 | FileCheck -check-prefix=GISEL-ERR %s
; RUN: not llc -global-isel=1 -new-reg-bank-select -mtriple=amdgcn -mcpu=gfx900 -filetype=null %s 2>&1 | FileCheck -check-prefix=GISEL-ERR %s
...
; GISEL-ERR: LLVM ERROR: unable to legalize instruction: %{{[0-9]+}}:_(<4 x f32>) = G_AMDGPU_INTRIN_IMAGE_LOAD intrinsic(@llvm.amdgcn.image.sample.d.1d), 15, %{{[0-9]+}}:_(f16), %{{[0-9]+}}:_(f16), %{{[0-9]+}}:_(f32), %{{[0-9]+}}:_(<8 x i32>), %{{[0-9]+}}:_(<4 x i32>), 0, 0, 0 :: (dereferenceable load (<4 x f32>), addrspace 8) (in function: sample_d_1d)
